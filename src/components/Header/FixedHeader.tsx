...
export function FixedHeader() {
    const { isBurgerOpen, toggleBurger, isMobile, setIsMobile } = useBurgerStore();
    ...
    return (
        <header className={`bg-[var(--color-fixed)] fixed z-10 top-0 left-0 transitioned p-[31px] 
        ...
        ${isMobile && isBurgerOpen ? " h-screen" : ""}`}>
            ...
            {isMobile && (
                ...
                ${isBurgerOpen ? "pb-[30px] border-[var(--color-text)]" : "border-transparent"}`}>
                    <h2 className="roboto font-bold text-3xl flex items-center gap-2">
                        ...
                        <span className="text-[var(--color-text)]">FinTrack</span>
                    </h2>
                    <button aria-label="Toggle menu" onClick={toggleBurger} className="group w-[36px] rounded-lg border-0 cursor-pointer">
            ...
            <div className={`flex flex-col md:justify-between max-md:gap-10 max-md:my-5 h-full transitioned 
