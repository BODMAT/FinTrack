// TEST ONLY (DEV BEFORE BACKEND IS READY)
type CustomDate = "day" | "week" | "month" | "year" | "all";
interface IData {
    userId: string;
    id: string;
...
type MoneyType = "income" | "outcome" | "saving" | "balance";

function groupData(data: IData[], range: CustomDate, nowDate?: Date) {
...
function getTotalOfRange(data: IData[], range: CustomDate, title: MoneyType, nowDate?: Date): number {
    if (title === "balance") {
...
function getStatsPerRange(data: IData[], range: CustomDate): IDataStatsPerRange {
    const totalIncomePerRange = getTotalOfRange(data, range, "income");
    const percentageIncomePerRange = getPercentageOfRangeIncrease(data, range, "income");
    const totalOutcomePerRange = getTotalOfRange(data, range, "outcome");
    const percentageOutcomePerRange = getPercentageOfRangeIncrease(data, range, "outcome");
    const totalSavingPerRange = getTotalOfRange(data, range, "saving");
    const percentageSavingPerRange = getPercentageOfRangeIncrease(data, range, "saving");
