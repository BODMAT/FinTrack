...
export function TransactionsCard({ data }: { data: IData }) {
    ...
    return (
        ...
        >
            ...
            <span className={`justify-self-center py-2 px-5 text-sm rounded-xl text-gray-40 ${data.isIncome ? "bg-[var(--bg-green)]" : "bg-[var(--bg-red)]"}`}>
                {data.isIncome ? "Income" : "Outcome"}
            </span>
            <span
            ...
            <span className="text-center col-span-2 max-[1300px]:col-span-1 justify-self-center px-2 break-words whitespace-normal">{data.location ? `${data.location.lat.toFixed(3)}, ${data.location.lng.toFixed(3)}` : "No location"}</span>
